...
description: Real audit of the agent-gate component — coverage, known gaps, stale docs
user-invocable: true
---

# Audit Component (`agent-gate`)

1. Ensure `/audit-repo-access` passed (or run it).
2. Clone/fetch into `~/repos/aegis` (read). Focus on `agent-gate/` only.
...
7. If `mcp__trinity__report` available: `report_type: aegis_agent_gate.component_audit`.
